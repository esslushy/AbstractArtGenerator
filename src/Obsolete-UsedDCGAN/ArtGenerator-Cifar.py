...
config = tf.ConfigProto(intra_op_parallelism_threads=2, inter_op_parallelism_threads=1, allow_soft_placement=True, log_device_placement=True)#last allows for gpu settings
...
with tf.Session(config=config) as sess:
    writer = tf.summary.FileWriter("./infoCifar", sess.graph)
    ...
    for epoch in range(numEpochs):
        for numBatch, (realData, _) in enumerate(dataLoader):
            ...
            realData = realData.permute(0, 2, 3, 1).numpy() #turns them into 64x64x3 and turns them into a numpy  array
            ...
            if numBatch % 10 == 0:
                ...
                i+=1
            if numBatch % 100 == 0:
                saver.save(sess, "./modelCifar/DCGAN_Epoch_%s_Batch_%s.ckpt" % (epoch, numBatch))
