...
discriminatorLossReal = tf.reduce_mean(
                           nn.sigmoid_cross_entropy_with_logits(
                               logits=discriminatorRealLogits, labels=.9,#suggested smoothing
...
discriminatorLossFake = tf.reduce_mean(
                           nn.sigmoid_cross_entropy_with_logits(
                               logits=discriminatorFakeLogits, labels=0.,
...
generatorLoss = tf.reduce_mean(
                           nn.sigmoid_cross_entropy_with_logits(
                               logits=discriminatorFakeLogits, labels=1.,
                               name="generator_loss"
...
#gradient ascent on discriminator loss, so negative loss value is the same as ascent
discriminatorOptimizer = tf.train.AdamOptimizer(0.00015).minimize(discriminatorTotalLoss*-1, var_list=dTrainableVariables)
#gradient descent on generator loss
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(numEpochs):
        for numBatch, (realData, _) in enumerate(dataLoader):
            ...
            _, _, _, dLoss, dRealPred, dFakePred, = sess.run([discriminatorOptimizer, discriminatorTotalLoss, discriminatorReal, discriminatorFake], 
