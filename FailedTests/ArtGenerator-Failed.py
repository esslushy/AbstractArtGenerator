...
batchSize = 10 #each batch will have 10 images. needs to be small because it overloads gpu
numEpochs = 200#number of training epochs
showBatchSize = 16#number of test images

#prepare data
dataset = loadImages("ImagesX64.npy")
batchedDataset = dataset.shuffle(93900).batch(batchSize)
...
with tf.device("/gpu:0"):
    ...
    discriminator = makeDiscriminator(images)#make discriminator. logits aka output
...
def train(sess, realData, fakeData, optimizer, discriminator, trainableVariables, gOptimizer, generator, gTrainableVariables):
    ...
    sess.run(optimizer.minimize(totalLoss, var_list=trainableVariables))
    ...
    #backpropagate and update weights
    sess.run(gOptimizer.minimize(totalGLoss, var_list=gTrainableVariables))
